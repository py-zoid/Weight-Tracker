...
class UserRegistrationBloc extends Bloc<RegisterUserEvent, UserRegistrationState>{
  ...
  @override
  Stream<UserRegistrationState> mapEventToState(RegisterUserEvent event) async*{
    if (event is RegisterUserEvent){
      yield UserRegistrationState.loading();
      try {
        UserObject user = await _userRepository.signUpEmail(event.email, event.password, event.name);
